...
export default function ScreeningNurseTopbar({ nurseLabel, initials, live }) {

  return (
    <header className={`${topbar.root} shrink-0`}>
      <div className="flex min-w-0 flex-col sm:flex-row sm:items-baseline sm:gap-2">
        <span className={topbar.brand}>E-Health Management system</span>
        <span className="text-sm font-medium text-slate-500">Screening Nurse · Clinical screening</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 sm:gap-4">
        ...
        <TopbarSignOutButton moduleLabel='Screening Nurse' className={topbar.signOut} />
      </div>
    </header>
  );
}
