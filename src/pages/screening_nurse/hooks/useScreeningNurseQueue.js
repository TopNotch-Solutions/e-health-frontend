...
const DEPT = 'screening_nurse';

export function useScreeningNurseQueue({ onQueueSynced } = {}) {
...
export function useScreeningNurseSession() {
  const user = getStoredUser();
  const nurseLabel =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'Screening Nurse';
  const initials =
