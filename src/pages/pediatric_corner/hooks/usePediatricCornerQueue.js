...
import { pediatricEligibility } from '../pediatricCornerForm';

const DEPT = 'pediatric';

export function usePediatricCornerQueue({ onQueueSynced } = {}) {
  ...
  const applyEntries = useCallback((entries) => {
    const mapped = filterActiveQueueEntries(entries).map((entry) => {
      const base = mapQueueEntry(entry);
      const elig = pediatricEligibility(base.patient);
      return { ...base, pediatricEligible: elig.eligible, patientAge: elig.age };
    });
    setQueue(mapped);
  ...
  const loadQueueHttp = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load Pediatric Corner queue');
      setQueue([]);
      return [];
    }
  }, [applyEntries]);
  ...
  useEffect(() => {
    ...
    const onConnect = () => { setLive(true); requestDepartmentQueueRefresh(DEPT); };
...
export function usePediatricCornerSession() {
  const user = getStoredUser();
  const label =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'Pediatric Corner';
  const initials =
    label.split(/\s+/).filter(Boolean).slice(0, 2).map((w) => w[0]?.toUpperCase() || '').join('')
    || 'PC';
  return { user, label, initials, userId: user?.id ?? null };
}
