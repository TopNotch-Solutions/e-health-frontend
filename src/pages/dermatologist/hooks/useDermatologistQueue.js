...
const DEPT = 'dermatologist';

export function useDermatologistQueue({ onQueueSynced } = {}) {
  ...
  const loadQueueHttp = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load dermatologist queue');
      setQueue([]);
      return [];
    }
  }, [applyEntries]);
...
export function useDermatologistSession() {
  const user = getStoredUser();
  const label =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'Dermatologist';
  const initials =
