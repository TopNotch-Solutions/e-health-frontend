...
export default function BillingFeeEditor({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setFees(rows);
      setDraftAmounts(Object.fromEntries(rows.map((row) => [row.fee_key, String(row.amount)])));
              ...
              <table className={c.whiteTable}>
                <thead>
                  <tr>
                    ...
                    {showNationalColumn ? <th className={c.whiteTh}>National (NAD)</th> : null}
                    {showEffectiveColumn ? <th className={c.whiteTh}>{currentColumnLabel}</th> : null}
                    {showOverrideColumn ? <th className={c.whiteTh}>Override (NAD)</th> : null}
                    <th className={c.whiteTh}>{newPriceColumnLabel}</th>
                ...
                <tbody>
                  {fees.map((fee) => (
                    <tr key={fee.fee_key}>
                      ...
                      {showNationalColumn ? (
                        <td className={c.whiteTdMuted}>{formatMoney(fee.national_amount)}</td>
                      ) : null}
                      {showEffectiveColumn ? (
                      ...
                      {showOverrideColumn ? (
                        <td className={c.whiteTdMuted}>
                          {fee.has_override ? formatMoney(fee.override_amount) : '—'}
                        </td>
                      ) : null}
                      <td className={c.whiteTd}>
                        <input
                ...
                <tbody>
                  {history.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className={c.whiteTdMuted}>{formatMoney(item.previous_amount)}</td>
                      <td className={c.whiteTd}>{formatMoney(item.new_amount)}</td>
