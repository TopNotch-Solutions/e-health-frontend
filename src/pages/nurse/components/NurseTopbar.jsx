/* topbar-signout-v2 */
import { topbar } from '../styles/nurseClasses';
import TopbarSignOutButton from '../../../components/TopbarSignOutButton';
import AppBrand from '../../../components/brand/AppBrand';

export default function NurseTopbar({ nurseLabel, initials, live }) {

  return (
    <header className={`${topbar.root} shrink-0`}>
      <div className="flex min-w-0 flex-col sm:flex-row sm:items-baseline sm:gap-2">
        <AppBrand className={topbar.brand} />
        <span className="text-sm font-medium text-slate-500">Nurse · Vitals intake</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 sm:gap-4">
        ...
        <TopbarSignOutButton moduleLabel='Nurse · Vitals intake' className={topbar.signOut} />
      </div>
    </header>
  );
}
