...
const DEPT = 'art_nurse';

export function useArtNurseQueue({ onQueueSynced } = {}) {
  ...
  const loadQueueHttp = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load ART queue');
      setQueue([]);
      return [];
    }
  }, [applyEntries]);
...
export function useArtNurseSession() {
  const user = getStoredUser();
  const nurseLabel =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'ART Nurse';
  const initials =
