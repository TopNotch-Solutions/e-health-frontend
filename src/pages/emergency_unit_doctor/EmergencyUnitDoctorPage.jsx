...
import {
  ...
  pickAutoResumeEntry,
} from './hooks/useEmergencyUnitDoctorQueue';
...
export default function EmergencyUnitDoctorPage() {
  ...
  const { queue, setQueue, loading, error: queueLoadError, live, refresh } = useEmergencyUnitDoctorQueue({
    onQueueSynced,
  });
    ...
    <div className={c.page}>
      ...
      <div className={c.body}>
        ...
        <aside className={c.queueAside} aria-label="Emergency doctor queue">
          ...
          {workspaceActive ? (
            <p className={c.queueSub}>One patient at a time — prescribe to pharmacy or route to Booking Room</p>
          ) : (
          ...
          {workspaceActive ? (
            <ActiveSessionQueueAside
              ...
              message="Review the clinical timeline, then prescribe to pharmacy or transfer to Booking Room."
            ...
            <>
              <div className={c.searchWrap}>
                <label htmlFor="eud-queue-search" className="sr-only">Search queue</label>
                <input
                  id="eud-queue-search"
                  type="search"
              ...
              <div className={c.queueList}>
                {loading ? (
                  ...
                  <p className={c.hint}>
                    {queueSearch.trim() ? 'No patients match your search.' : 'No patients in the emergency doctor queue.'}
                  </p>
                ) : (
                  filteredQueue.map((p) => (
                    <article
        ...
        <div className={c.main}>
          {!workspaceActive ? (
            <div className={c.idle} role="region" aria-label="Emergency doctor workspace">
              <QueueEmptyIcon />
              <h3 className={c.idleTitle}>No patient selected</h3>
              <p className={c.idleText}>
                Select a patient from the queue. Prescribe to pharmacy only, or transfer to Booking Room when administrative disposition is needed.
              </p>
            </div>
          ) : (
