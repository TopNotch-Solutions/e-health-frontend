...
export default function KitchenManagerPage() {
  ...
  const label =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'Kitchen manager';
  const initials =
    label
      .split(/\s+/)
      .filter(Boolean)
  ...
  return (
    <div className={c.page}>
      <KitchenTopbar
        ...
        initials={initials}
        live={live}
        moduleTag="Kitchen manager · Operations"
      />
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
          <header className="mb-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Inpatient diets prescribed today — track preparation and delivery by ward, room, and bed.
            </p>
          </header>
          <KitchenMealBoard
            mealData={mealData}
