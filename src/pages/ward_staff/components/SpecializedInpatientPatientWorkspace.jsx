...
import {
  dischargeSpecializedInpatientPatient,
  getAvailableBeds,
  getSpecializedInpatientDailyRecords,
  saveSpecializedInpatientDailyRecord,
  transferSpecializedInpatientToMortuary,
  transferSpecializedInpatientToWard,
} from '../../../api/ward';
import { confirmAction, confirmSpecializedInpatientDailySave } from '../../../utils/confirmAction';
import { wst } from '../styles/wardStaffClasses';
import SpecializedInpatientVitalsForm, { EMPTY_SI_VITALS } from './SpecializedInpatientVitalsForm';
...
} from '../specializedInpatientWardValidation';
...
function todayRecord() {
  return { ...EMPTY_SI_VITALS, record_date: todayIsoDate() };
}

export default function SpecializedInpatientPatientWorkspace({
  admission,
  actionLoading,
  setActionLoading,
  onToast,
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      const data = await getSpecializedInpatientDailyRecords(admission.id);
  ...
  async function handleSaveRecord() {
    ...
    const errors = validateSpecializedInpatientDailyRecord(payload);
    setVitalsErrors(errors);
    if (Object.keys(errors).length) {
      onActionError(firstValidationMessage(errors));
      return;
    }
    if (!(await confirmSpecializedInpatientDailySave(patientName))) return;

    setActionLoading(true);
    onActionError('');
    try {
      await saveSpecializedInpatientDailyRecord(admission.id, payload);
  ...
  async function handleTransferWard() {
    ...
    const errors = validateSpecializedInpatientPorterTransport(porterValidationInput());
    ...
    setActionLoading(true);
    try {
      const result = await transferSpecializedInpatientToWard(admission.id, {
  ...
  async function handleDischarge() {
    ...
    const errors = validateSpecializedInpatientDischarge({ dischargeNotes });
    ...
    if (!(await confirmAction({
      ...
      text: `Discharge ${patientName} from specialized inpatient? The bed will be freed immediately.`,
      icon: 'question',
    ...
    setActionLoading(true);
    try {
      const result = await dischargeSpecializedInpatientPatient(admission.id, {
  ...
  async function handleTransferMortuary() {
    ...
    const errors = validateSpecializedInpatientMortuaryTransfer({
      causeOfDeath,
    ...
    setActionLoading(true);
    try {
      await transferSpecializedInpatientToMortuary(admission.id, {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      <section className={wst.placementCard}>
        <h3 className={wst.placementTitle}>Specialized inpatient placement</h3>
        <p className={wst.placementMain}>{ward.name || 'Specialized inpatient'}</p>
      ...
      <section className={wst.sectionPanel}>
        ...
        <SpecializedInpatientVitalsForm
          vitals={vitals}
