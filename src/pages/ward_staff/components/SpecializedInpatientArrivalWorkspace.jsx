...
import { confirmSpecializedInpatientArrivalAndSave } from '../../../utils/confirmAction';
...
import { firstValidationMessage, validateSpecializedInpatientDailyRecord } from '../specializedInpatientWardValidation';
...
function todayRecord() {
  return { ...EMPTY_SI_VITALS, record_date: new Date().toISOString().slice(0, 10) };
}

export default function SpecializedInpatientArrivalWorkspace({
  admission,
  actionLoading,
  setActionLoading,
  onToast,
  ...
  async function handleConfirmArrivalAndSave() {
    ...
    if (!(await confirmSpecializedInpatientArrivalAndSave(patientName))) return;

    setActionLoading(true);
    onActionError('');
    try {
      await confirmPatientArrival(admission.id, { specialized_inpatient_record: payload });
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      <section className={wst.placementCard}>
        ...
        <p className={wst.placementMain}>{ward.name || 'Specialized inpatient'} ({ward.ward_number || '—'})</p>
