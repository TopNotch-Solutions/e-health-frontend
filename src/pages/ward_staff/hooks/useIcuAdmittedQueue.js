...
import { getIcuAdmittedPatients } from '../../../api/ward';
...
export function useIcuAdmittedQueue() {
  ...
  const loadQueueHttp = useCallback(async () => {
    setError('');
    try {
      const rows = await getIcuAdmittedPatients();
      setQueue((Array.isArray(rows) ? rows : []).map(mapRow));
      return rows;
    } catch (err) {
      setError(err.message || 'Failed to load ICU patients');
      setQueue([]);
      return [];
    }
  }, []);
