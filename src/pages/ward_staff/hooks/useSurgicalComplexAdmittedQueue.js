...
import { getSurgicalComplexAdmittedPatients } from '../../../api/ward';
...
export function useSurgicalComplexAdmittedQueue() {
  ...
  const loadQueueHttp = useCallback(async () => {
    setError('');
    try {
      const rows = await getSurgicalComplexAdmittedPatients();
      setQueue((Array.isArray(rows) ? rows : []).map(mapRow));
      return rows;
    } catch (err) {
      setError(err.message || 'Failed to load surgical complex patients');
      setQueue([]);
      return [];
    }
  }, []);
