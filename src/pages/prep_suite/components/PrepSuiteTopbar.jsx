...
export default function PrepSuiteTopbar({ nurseLabel, initials, live }) {

  return (
    <header className={`${topbar.root} shrink-0`}>
      <div className="flex min-w-0 flex-col sm:flex-row sm:items-baseline sm:gap-2">
        <AppBrand className={topbar.brand} />
        <span className="text-sm font-medium text-slate-500">PrEP Suite · Pre-exposure prophylaxis</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 sm:gap-4">
        ...
        <TopbarSignOutButton moduleLabel='PrEP Suite' className={topbar.signOut} />
      </div>
    </header>
  );
}
