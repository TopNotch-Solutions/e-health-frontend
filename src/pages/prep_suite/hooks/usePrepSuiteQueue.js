...
export function usePrepSuiteQueue({ onQueueSynced } = {}) {
  ...
  const loadQueueHttp = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load PrEP suite queue');
      setQueue([]);
      return [];
    }
  }, [applyEntries]);
...
export function usePrepSuiteSession() {
  const user = getStoredUser();
  const nurseLabel =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'PrEP Suite';
  const initials =
