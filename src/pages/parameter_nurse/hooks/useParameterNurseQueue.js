...
const DEPT = 'parameter_nurse';

export function useParameterNurseQueue({ onQueueSynced } = {}) {
...
export function useParameterNurseSession() {
  const user = getStoredUser();
  const nurseLabel =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'Parameter Nurse';
  const initials =
