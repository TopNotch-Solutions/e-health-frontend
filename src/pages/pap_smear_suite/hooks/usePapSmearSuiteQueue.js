...
const DEPT = 'pap_smear';

export function usePapSmearSuiteQueue({ onQueueSynced } = {}) {
  ...
  const loadQueueHttp = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load Pap Smear suite queue');
      setQueue([]);
      return [];
    }
  }, [applyEntries]);
...
export function usePapSmearSuiteSession() {
  const user = getStoredUser();
  const label =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'Pap Smear Suite';
  const initials =
