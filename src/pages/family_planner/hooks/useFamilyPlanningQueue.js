...
const DEPT = 'family_planning';

export function useFamilyPlanningQueue({ onQueueSynced } = {}) {
  ...
  const loadQueueHttp = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load Family Planning suite queue');
      setQueue([]);
      return [];
    }
  }, [applyEntries]);
...
export function useFamilyPlanningSession() {
  const user = getStoredUser();
  const label =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'Family Planner';
  const initials =
