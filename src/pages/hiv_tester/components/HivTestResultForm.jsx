...
import { confirmButtonClass, confirmButtonLabel } from '../hivTesterForm';
...
export default function HivTestResultForm({
  ...
  const canSubmit = form.result === 'negative' || form.result === 'positive';

  return (
    <section className={c.sectionPanel}>
      <h3 className={c.sectionTitle}>HIV test result</h3>
      <p className="mt-1 text-sm text-slate-500">
        Record the rapid test outcome. Negative ends this visit and saves to patient history.
        Positive automatically escalates the patient to the ART queue.
      </p>
      ...
      <fieldset className="mt-4">
        ...
        <div className="mt-3 flex flex-wrap gap-3">
          {[
            { value: 'negative', label: 'Negative', desc: 'End session · save to history' },
            ...
            >
              <input
                ...
                onChange={() => onFieldChange('result', opt.value)}
              />
      ...
      {form.result ? (
        ...
        <p className={`${c.hint} mt-6`}>Select a test outcome above to enable confirmation.</p>
      )}
