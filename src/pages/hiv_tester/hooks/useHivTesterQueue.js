...
const DEPT = 'hiv_tester';

export function useHivTesterQueue({ onQueueSynced } = {}) {
  ...
  const loadQueueHttp = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load HIV testing queue');
      setQueue([]);
      return [];
    }
  }, [applyEntries]);
...
export function useHivTesterSession() {
  const user = getStoredUser();
  const nurseLabel =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'HIV Tester';
  const initials =
