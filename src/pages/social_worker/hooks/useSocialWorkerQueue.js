...
const DEPT = 'social_worker';

export function useSocialWorkerQueue({ onQueueSynced } = {}) {
  ...
  const loadQueueHttp = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load Social Worker suite queue');
      setQueue([]);
      return [];
    }
  }, [applyEntries]);
...
export function useSocialWorkerSession() {
  const user = getStoredUser();
  const label =
    [user?.first_name, user?.last_name].filter(Boolean).join(' ').trim() || 'Social Worker';
  const initials =
