...
export default function SocialWorkerSuiteTopbar({ label, initials, live }) {
    ...
    <header className={`${topbar.root} shrink-0`}>
      <div className="flex min-w-0 flex-col sm:flex-row sm:items-baseline sm:gap-2">
        ...
        <span className="text-sm font-medium text-slate-500">
          Social Worker Suite · Psychosocial counseling
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 sm:gap-4">
        ...
        <TopbarSignOutButton moduleLabel='Social Worker Suite' className={topbar.signOut} />
      </div>
    </header>
  );
}
